...
public:
    ...
    [[nodiscard]] constexpr uint8_t read(const uint &i) const {
        storage_type word = sbb::runtime_index_wrapper(storage_array_, i / sizeof(storage_type));
    ...
    constexpr uint8_t write(const uint &i, const uint8_t &write_byte) {
        return sbb::runtime_index_wrapper_store_byte(storage_array_, i / sizeof(storage_type), write_byte, i % sizeof(storage_type));
    }
